...
class ProbeInfo:
    ...
    def dump(self, file=sys.stdout) -> None:
        ...
        print(
            f"Hardware config: [{self.__hardware_config}]", file=file, flush=True)
        print(
            f"Current ticks per amp: [{self.__current_ticks_per_amp}]", file=file, flush=True)
        print(
            f"Time ticks per sec: [{self.__time_ticks_per_sec}]", file=file, flush=True)
        print(
            f"Histogram bucket steps/sec: [{self.__histogram_bucket_steps_per_sec}]", file=file, flush=True)
