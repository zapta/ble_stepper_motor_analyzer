...
class Probe:
    ...
    async def write_command_set_nickname(self, nickname,  response=False):
        if not self.is_connected():
        ...
        await self.__client.write_gatt_char(self.__stepper_command_chrc, cmd_bytes, response=response)
